...
class CoverImageTest {
    ...
    void 커버이미지의_정해진_크기_이하이면_에러전파() {
        assertThatThrownBy(() -> {
            new CoverImage(1_500_000, CoverImageType.JPEG, 299, 200);
        }).isInstanceOf(CanNotCreateException.class)
    ...
    void 커버이미지의_정해진_비율이_아니면_에러전파() {
        assertThatThrownBy(() -> {
            new CoverImage(1_500_000, CoverImageType.JPEG, 300, 199);
        }).isInstanceOf(CanNotCreateException.class)
    ...
    void 커버이미지의_너비와_높이_비율은_3대2가_아니면_에러전파() {
        assertThatThrownBy(() -> {
            new CoverImage(1_500_000, CoverImageType.JPEG, 300, 300);
        }).isInstanceOf(CanNotCreateException.class)
