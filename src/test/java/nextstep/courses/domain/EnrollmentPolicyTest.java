...
class EnrollmentPolicyTest {
    ...
    void 유료인데_수강료와_지불금액이_다르면_에러전파() throws Exception {
        assertThatNoException().isThrownBy(() -> {
            new EnrollmentPolicy(10, 10L).validatePayment(new Payment());
        });
    }
    ...
    void 무료인데_수강료_납부하면_에러전파() throws Exception {
        ...
        assertThatThrownBy(() -> {
            enrollmentPolicy.validatePayment(new Payment());
    ...
    void 수강신청_인원이_초과하면_에러전파() throws Exception {
        EnrollmentPolicy enrollmentPolicy = new EnrollmentPolicy(10L, 10, new EnrolledUsers(10), new SessionStatus());
        assertThatThrownBy(() -> {
            enrollmentPolicy.validateEnrollment();
        }).isInstanceOf(CanNotJoinException.class)
    ...
    void 무료강의인데_수강신청_시_정원체크하면_에러전파() throws Exception {
        EnrollmentPolicy enrollmentPolicy = new EnrollmentPolicy(new EnrolledUsers(10), new SessionStatus());
        assertThatThrownBy(() -> {
            enrollmentPolicy.validateEnrollment();
        }).isInstanceOf(CanNotJoinException.class)
